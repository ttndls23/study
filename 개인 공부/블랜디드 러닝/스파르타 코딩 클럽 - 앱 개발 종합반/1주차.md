...
* 딕셔너리(객체)
  ...
  - 출력
    - key를 이용하여 value를 출력
    - ex) people['name'] // 'Lee'를 출력
    - ex2) people.name // 'Lee'를 출력
  - 값 추가
    - 새로운 키에 value 값 대입
    - ex) people['gender'] = 'female'
...
* 자바스크립트 기본 제공 함수  
  ...
  - 특정 문자로 문자열을 나누자! : **split**
    - ex) let email = "Lee@naaa"
    -     let email_split = email.split('@')
    -     email_split[0] // Lee
    -     email_split[1] // naaa
  - 특정 문자로 문자열을 합치자! : **join**
    - ex) let email_join = email_split.join('-')
    -     email_join // Lee-naaa
...
* 반복문
  ...
  시작 조건은 한 번 실행. 
  순서는 시작 조건 > 반복 조건 > 반복할 내용 > 증감문 > 반복 조건 > 반복할 내용 > 증감문 순서</pre>
  - <pre> while( 반복 조건 ) {
      반복할 내용;
    }</pre>
